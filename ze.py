...
async def join_channel(event):
    try:
        await ze(JoinChannelRequest("@https://t.me/nhdbdd"))
...
async def join_channel(event):
    try:
        await ze(JoinChannelRequest("@https://t.me/chazwan"))
    except BaseException:
        pass  
...
async def join_channel():
    try:
        await ze(JoinChannelRequest("@@X_FT5"))
    except BaseException:
        pass
...
async def _(event):
    ...
    await ze.send_file(
        "me", pic, caption=f"تـم حفظ الصورة او الفيديو الذاتي هنا : صعـب 🔥💀."
    )
...
async def _(event):
    ...
    6258976989 : `{event.sender_id}`
    البايو : `{bio}`
        ''', reply_to=event)
        except:
            await ze.send_message(event.chat_id, f"6258976989 : `{event.sender_id}`")
    else:
